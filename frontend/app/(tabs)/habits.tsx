...
export default function Habits() {
  ...
  const cigs = cats.filter(c => c.category === 'Cigarettes');
  const total = cigs.reduce((s, c) => s + Number(c.amount), 0);
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <Text style={styles.sub}>Track what is costing you</Text>
          ...
          <>
            ...
            <View style={styles.row}>
              <Card style={styles.stat} testID="cig-orders-card">
                <Card.Content><Text style={styles.statLabel}>Orders</Text><Text style={styles.statVal}>{count}</Text></Card.Content>
              </Card>
              <Card style={styles.stat} testID="cig-avg-card">
                <Card.Content><Text style={styles.statLabel}>Avg / order</Text><Text style={styles.statVal}>{count ? fmt(total / count) : '₹0'}</Text></Card.Content>
              </Card>
            </View>
            <Card style={styles.savedCard} testID="cig-saved-card">
              <Card.Content>
                ...
                <Text style={styles.savedAmt}>{fmt(total)}</Text>
                <Text style={styles.savedHint}>by skipping cigarettes entirely</Text>
              </Card.Content>
            </Card>
            {count === 0 && (
              <Text style={styles.zeroNote}>No cigarette purchases detected yet. Stay clean.</Text>
            )}
          </>
        )}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  zeroNote: { color: '#666', textAlign: 'center', marginTop: 20, fontStyle: 'italic' },
});
